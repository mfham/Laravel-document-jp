...
Laravelフレームワークによって作られたすべてのクッキーは暗号化されており、認証コードで署名されています。それがクライアントによって変えられたら無効として考えられます。リクエストからクッキー値を取得するために、`Illuminate\Http\Request`インスタンスの`cookie`メソッドを使えます。
```php
